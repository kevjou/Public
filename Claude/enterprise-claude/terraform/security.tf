# ── KMS ─────────────────────────────────────────────────────────────────────
# Single CMK for all resources. HIPAA requires annual rotation (enabled below).

resource "aws_kms_key" "main" {
  description             = "${local.name_prefix} HIPAA encryption key"
...
resource "aws_security_group" "ecs" {
  ...
  description = "ECS proxy: inbound from ALB only"
  vpc_id      = aws_vpc.main.id
  ...
  egress {
    description = "Outbound to Anthropic API and AWS services"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
# ── IAM: ECS Execution Role (pull image, fetch secrets) ─────────────────────
